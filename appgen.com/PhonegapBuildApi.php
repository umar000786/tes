...
class PhonegapBuildApi
{
    ...
    public function __construct($usernameOrToken = '', $password = '')
    {
        ...
        if (! empty($usernameOrToken)) 
        {
            if (! empty($password)) {
                echo "enter cr";
               $mycredntials = $this->setCredentials($usernameOrToken, $password);
     ...
     * @return PhonegapBuildApi
     */
     public function getcredantials()
     {
         $this->username;
        $this->password; 
     ...
     */
    public function request($uri, $method = 'get', $options = array())
    {
        $this->clear();
